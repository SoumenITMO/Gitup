...
# PROCESSING ... 20140219.export.CSV.zip
# [u'This+photo+went+viral+as+showing+a+Syria+refugee+child+who+had+lost+his+family.+Thousands+of+retweets+later+it+turned+out+to+be+wrong.']
# db.getCollection('Syria_Refugee').find({"Twitter_data_.geo":{$ne:null}, "Twitter_data_.text":{$regex:/refugee/}}).count()
...
skip_words_list = ["a", "in", "offer", "In", "do", "must", "the", "on", "before", "round", "upon", "come", "to", "us",
...
t = [u'4-Year-Old', u'Syrian', u'Refugee', u'Found', u'Wandering', u'In']
                    ...
                    if re.match(r'^((http://)|(https://)).*?[Rre]fuge.*', urls[0]):
                        ...
                        if twitt_key != []:
                               ...
                               if "%27beirut,%20lb%27%29&format=" not in twitt_key[0] \
                                    ...
                                    if "&" in twitter_query:
                                        modify_ = urllib2.unquote(twitter_query).decode('utf-8', 'ignore').split("&")[0]
